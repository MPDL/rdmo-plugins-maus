...
class SMPRepoImportMixin(ProjectImportMixin, RDMOXMLImport):
    ...
    def merge_languages(self, new_language_values, import_values):
        ...
        for i, v in enumerate(unique_new_language_values):
            ...
            if language not in [l.lower() for (j, l) in import_values_languages]:
    ...
    def merge_dependencies(self, new_dependencies_values, import_values):
        ...
        If there is a dependecies value in the project, return a value that merges its text with the imported information.
        Otherwise, return a value with the imported information.
    ...
    def merge_dependency_licenses(self, new_dependency_licenses_values, import_values):
        ...
        If there is a dependecy licenses value in the project, return a value that merges its text with the imported information.
        Otherwise, return a value with the imported information.
    ...
    def merge_contributors(self, new_contributor_values, import_values):
        ...
        Because:
        - The orcid for the imported "Muriel P. Musterfrau" matches the corresponding orcid  
          of the "Muriel Musterfrau" in the project (imported values of "Muriel P. Musterfrau" get index 0 instead of 1).
    ...
    def get_import_values(self, headers, request_urls):
        ...
        for import_option_key, import_option_url in request_urls.items():
            imports, process_method, process_method_kwargs = self.smp_import_map.get(import_option_key).values()
    ...
    def create_import_project(self, headers, xml_url, default_title):
        '''Return a Project() instance that will be the basis to create an xml with all the info from the selected repository. 
